...
const apiList = [
  {
    title: 'Get data by query',
    apiUrl: '/apis/${module}/index',
    details: 'method type is post ',
    example: '',
  },
  {
    title: 'Get data by Id',
    apiUrl: '/apis/${module}/findById/${id}',
    details: 'method type is post ',
    example: '',
  },
  {
    title: 'Insert data',
    apiUrl: '/apis/${module}/insert',
    details: 'method type is post ',
    example: '',
  },
  {
    title: 'Delete by Id',
    apiUrl: '/apis/' + '${module}' + '/delete/' + '${id}',
    details: 'method type is post ',
    example: '',
  },
  {
    title: 'Delete by Condition',
    apiUrl: '/apis/${module}/delete',
    details: 'method type is post ',
    example: '',
  },
  {
    title: 'Update data with Condition',
    apiUrl: '/apis/${module}/update',
    details: 'method type is post ',
    example: '',
  },
  {
    title: 'Update by id',
    apiUrl: '/apis/${module}/update/${id}',
    details: 'method type is post ',
    example: '',
  },
];
    ...
    $.ajax({
      ...
      success: (returnData) => {
        if (returnData.status == 'success') {
          ...
          _this.setState(
            {
              tableName: returnData.data.tableName,
              columns: columns,
              fields: fields,
            },
            () => {
        ...
        <div>
          <div>
            <button
              ...
              onClick={this._openCreateDataModel}
            >
              Create Data
            </button>
          </div>
          {this.state.apiListOpen ? (
            <Model
              title={'Api list'}
              onCloseModel={() => {
                this.setState({ apiListOpen: false });
              }}
            >
              <div>
                {apiList.map((d) => {
          ...
          <div className="pull-right">
            <span label="show apis" onClick={this.showApiList}>
              <i
  ...
  render() {
    ...
    return (
      <div className="col-md-4 offset-md-4">
        ...
        <div>
          {!this.props.edit ? (
            <Button className="btn btn-md btn-primary" onClick={this.onCreate}>
              Create data
            </Button>
          ) : (
            <Button
