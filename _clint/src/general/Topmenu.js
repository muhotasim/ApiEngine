...
class Topmenu extends React.PureComponent {
  ...
  render() {
    ...
    return (
      <Fragment>
        <nav className="app-top-bar navbar navbar-expand-lg navbar-dark bg-primary">
          <button
...
const MenuItem = (props) => {
  ...
  if (menu.menus.length == 0) {
    return (
      <li className="nav-item active" key={uuid()}>
    ...
    return (
      <li className="nav-item dropdown" key={uuid()}>
        <a
        ...
        <div className="dropdown-menu">
          {menu.menus.map((sub_menu, index) => {
            return (
              <Link className="dropdown-item" to={sub_menu.link}>
                {sub_menu.label}
              </Link>
            );
          })}
