...
class Datatable extends React.Component {
    ...
    render() {
        ...
        return (<div className="custom-datatable-holder ">
            {exports == true ? <div>
                <button style={{
                    ...
                    fontWeight: "700"
                }} onClick={this._exportData} className={"btn btn-sm btn-default pull-right mt-2 mr-2"}>Export
                </button>
            </div> : null}
            <table className="table table-striped custom-datatable table-bordered table-sm">
                ...
                <tr>
                    {columns.map((cl, index) => {
                        return (<th key={uuid()} style={widths ? widths[index] : {}}>{cl.label}</th>)
                    })}
                </tr>
                </thead>
                <tbody style={{position: "relative"}}>
                <TableBody _onParamsChange={this._onParamsChange} datas={datas} loading={loading} columns={columns}/>
                </tbody>
            </table>
    ...
    if (loading) {
        return <Fragment>
            <tr className={"text-center"} style={{
                ...
                <td style={{border: "none", fontSize: "16px"}} className="fa fa-refresh fa-spin"></td>
            </tr>
        </Fragment>
    }
    return <Fragment>
        {datas.map((data, index) => {
            return <TrComp key={uuid()} columns={columns} _onParamsChange={_onParamsChange} data={data}/>
        })}
...
const TrComp = (props) => {
    ...
    return (<tr key={uuid()} onClick={() => {
        _onParamsChange(data, "row")
    }}>
        {columns.map((cl, index) => {
            if (cl.type == "data") {
                return (<td>{data[cl.column]}</td>)
            } else if (cl.type == "node") {
                return <td>
                    {data[cl.column].map((v, i) => {
                        return <span className={v.className} onClick={(e) => {
                            e.stopPropagation();
                            _onParamsChange(v.data, v.id)
                        }} dangerouslySetInnerHTML={{__html: v.label}}></span>
                    })}
                </td>;
            } else {
                return null;
